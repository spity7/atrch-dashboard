...
const HandizListTable = ({ handiz }) => {
  const { deleteHandiz } = useGlobalContext() // ✅ hook inside component

  const handleDelete = async (id) => {
    const result = await Swal.fire({
      title: 'Are you sure?',
      text: 'This will permanently delete the handiz!',
      icon: 'warning',
    ...
    if (result.isConfirmed) {
      try {
        await deleteHandiz(id)
        Swal.fire('Deleted!', 'handiz has been deleted.', 'success')
    ...
    },
    {
      ...
      cell: ({
        ...
        <div className="d-flex gap-2">
          <Link to={`/ecommerce/handiz/edit/${_id}`} className="btn btn-sm btn-soft-secondary" title="Edit Handiz">
            <IconifyIcon icon="bx:edit" className="fs-18" />
          </Link>
          <button type="button" className="btn btn-sm btn-soft-danger" title="Delete Handiz" onClick={() => handleDelete(_id)}>
...
export default HandizListTable
