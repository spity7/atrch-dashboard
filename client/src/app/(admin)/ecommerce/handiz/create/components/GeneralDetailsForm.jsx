...
const generalFormSchema = yup.object({
  title: yup.string().required('Handiz title is required'),
  descQuill: yup.string().required('Handiz description is required'),
})
...
const GeneralDetailsForm = () => {
  const { createHandiz } = useGlobalContext()
  ...
  const onSubmit = async (data) => {
    try {
      ...
      await createHandiz(formData)

      alert('Handiz created successfully!')
      ...
      alert(error?.response?.data?.message || '❌ Failed to create handiz')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      <Row>
        <Col lg={6}>
          <TextFormInput
            control={control}
            label="Handiz Title"
            placeholder="Enter handiz title"
      ...
      <Row>
        <Col lg={12}>
          <div className="mb-5 mt-3">
            <label className="form-label">Handiz Description</label>
            <Controller
              name="descQuill"
              control={control}
              render={({ field }) => (
      ...
      <Button type="submit" disabled={loading} className="mt-4">
        {loading ? 'Creating...' : 'Create Handiz'}
      </Button>
    </form>
  )
}
