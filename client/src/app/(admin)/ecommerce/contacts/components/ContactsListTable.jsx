...
const ContactsListTable = ({ contacts }) => {
  const { deleteContact } = useGlobalContext() // ✅ hook inside component
  ...
  const columns = [
    {
      ...
      cell: ({
        row: {
          original: { _id, thumbnailUrl },
        },
      }) => (
        <div className="d-flex align-items-center">
          <div className="flex-shrink-0 me-3">
            {thumbnailUrl ? (
              <img src={thumbnailUrl} alt={''} className="img-fluid avatar-sm" style={{ width: 50, height: 50, objectFit: 'contain' }} />
            ) : (
    ...
    },
    {
      ...
      cell: ({
        ...
        <div className="d-flex gap-2">
          <Link to={`/ecommerce/contacts/edit/${_id}`} className="btn btn-sm btn-soft-secondary" title="Edit Contact">
            <IconifyIcon icon="bx:edit" className="fs-18" />
          </Link>
          <button type="button" className="btn btn-sm btn-soft-danger" title="Delete Contact" onClick={() => handleDelete(_id)}>
